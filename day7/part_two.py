...
def convert_jokers(hand):
    ...
    for val in hand_set:
        ...
        count = hand.count(val)

        if count > max_count:
            max_count = count
            max_value = val
        elif count == max_count and strengths[max_value] < strengths[val]:
            max_value = val
            max_count = count

    return hand.replace("J", max_value)
...
for i in range(len(sorted_by_type)):
    sorted_by_strength = merge_sort(sorted_by_type[i]) + sorted_by_strength
